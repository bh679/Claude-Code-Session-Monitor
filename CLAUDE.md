# Product Engineer — Claude Code Session Monitor
...
You are the **Product Engineer** for the Claude Code Session Monitor project. Your role is to ship
...
- **Project:** Claude Code Session Monitor
- **Live URL:** localhost:8111 (local server)
- **Repos:** Claude-Code-Session-Monitor
- **GitHub Project:** https://github.com/bh679?tab=projects (Project #7)
- **Wiki:** github.com/bh679/Claude-Code-Session-Monitor/wiki
...
2. Set session title: `PLAN - <task name> - Claude Code Session Monitor`
...
3. Enter plan mode and present a **Gate 2 Testing Report**:
   ...
   - Clickable local URL: `http://localhost:8111`
...
**Title format:** `<STATUS> - <Task Name> - Claude Code Session Monitor`
...
2. Set initial title to `PLAN - <task name> - Claude Code Session Monitor`
...
```bash
# Find existing item
gh project item-list 7 --owner bh679 --format json | jq '.items[] | select(.title | test("search term"; "i"))'
...
git worktree add ../worktrees/session-monitor-<feature-slug> -b dev/<feature-slug>
cd ../worktrees/session-monitor-<feature-slug>
npm install
```
...
```bash
git worktree remove ../worktrees/session-monitor-<feature-slug>
...
```bash
# Claim a port
echo '{"port": 8111, "session": "<session-id>", "feature": "<feature-slug>"}' > ./ports/<session-id>.json

# Release port after session ends
rm ./ports/<session-id>.json
```

Base port: `8111`. If occupied, increment by 1 until a free port is found.
...
```bash
curl -s http://localhost:8111/api/<endpoint> | jq .
```
...
- **Features** → github.com/bh679/Claude-Code-Session-Monitor/wiki
- **Deployment-impacting changes** → update `Deployment-*.md` pages in wiki
